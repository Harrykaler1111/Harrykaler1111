...
# Instagram OAuth (Meta App credentials - MOCKED for demo)
...
ADMIN_JWT_EXPIRATION_HOURS = 8  # 8 hours for admin tokens

# Commission settings
DEFAULT_COMMISSION_RATE = 10.0  # 10%
MIN_WITHDRAWAL_AMOUNT = 1000  # ₹1000
...
def require_permission(resource: str, action: str):
    def decorator(func):
        ...
        async def wrapper(*args, **kwargs):
            ...
            if not check_permission(admin, resource, action):
...
async def credit_influencer_commission(influencer_id: str, order_id: str, order_total: float, commission_rate: float):
    """Credit commission to influencer wallet"""
    ...
    # Update influencer wallet and stats
...
@api_router.post("/admin/auth/login", response_model=Dict)
...
@api_router.get("/admin/auth/me", response_model=AdminUserResponse)
...
@api_router.post("/admin/users", response_model=AdminUserResponse)
...
@api_router.put("/admin/users/{admin_id}", response_model=AdminUserResponse)
...
@api_router.post("/admin/users/{admin_id}/toggle-2fa")
async def toggle_2fa(admin_id: str, admin: Dict = Depends(get_admin_user)):
    """Toggle 2FA for admin user"""
...
@api_router.put("/influencers/{influencer_id}/commission")
...
@api_router.get("/influencers/instagram/connect")
async def get_instagram_connect_url(user: Dict = Depends(get_current_user)):
    ...
    # For demo - return mock OAuth URL
    # In production, this would be the actual Meta/Facebook OAuth URL
    ...
    oauth_url = f"https://www.facebook.com/v20.0/dialog/oauth?client_id={INSTAGRAM_APP_ID}&redirect_uri={INSTAGRAM_REDIRECT_URI}&scope=instagram_basic,instagram_manage_messages,pages_messaging&response_type=code&state={state}"
    
    return {
        "oauth_url": oauth_url,
        "state": state,
        "note": "MOCKED - In production, this would redirect to actual Meta OAuth"
    }

@api_router.post("/influencers/instagram/callback")
async def instagram_oauth_callback(code: str, state: str, user: Dict = Depends(get_current_user)):
    ...
    # In production, exchange code for access token via Meta Graph API
    mock_ig_user_id = f"ig_{uuid.uuid4().hex[:10]}"
    mock_ig_username = influencer.get("instagram_handle", "").replace("@", "") or f"user_{uuid.uuid4().hex[:6]}"
    ...
    await db.influencers.update_one(
        ...
        {"$set": {
            ...
            "instagram_username": mock_ig_username,
            "instagram_access_token": f"mock_token_{uuid.uuid4().hex}",  # Encrypted in production
            "instagram_token_expires": (datetime.now(timezone.utc) + timedelta(days=60)).isoformat(),
            "updated_at": datetime.now(timezone.utc).isoformat()
        }}
    )
    
    await db.instagram_oauth_states.delete_one({"influencer_id": influencer["influencer_id"]})
    
    return {
        "success": True,
        "instagram_username": mock_ig_username,
        "message": "Instagram account connected successfully (MOCKED)"
    }

@api_router.post("/influencers/instagram/disconnect")
...
@api_router.post("/influencers/instagram/toggle-automation")
async def toggle_instagram_automation(enabled: bool, user: Dict = Depends(get_current_user)):
    """Enable/disable comment-to-DM automation"""
    influencer = await db.influencers.find_one({"user_id": user["user_id"]}, {"_id": 0})
    if not influencer:
        raise HTTPException(status_code=404, detail="Not registered as influencer")
...
@api_router.post("/influencers/instagram/posts", response_model=InstagramPostResponse)
async def register_instagram_post(post_data: InstagramPostCreate, user: Dict = Depends(get_current_user)):
    """Register an Instagram post for automation"""
    influencer = await db.influencers.find_one({"user_id": user["user_id"]}, {"_id": 0})
    if not influencer:
        raise HTTPException(status_code=404, detail="Not registered as influencer")
    ...
    post_record = {
        ...
        "total_comments": 0,
        "total_dms_sent": 0,
        "commented_users": [],  # Track users who received DMs to avoid duplicates
        "created_at": datetime.now(timezone.utc).isoformat(),
        "updated_at": datetime.now(timezone.utc).isoformat()
    }
...
@api_router.get("/influencers/instagram/posts", response_model=List[InstagramPostResponse])
async def get_instagram_posts(user: Dict = Depends(get_current_user)):
    """Get all registered Instagram posts"""
    influencer = await db.influencers.find_one({"user_id": user["user_id"]}, {"_id": 0})
    if not influencer:
        raise HTTPException(status_code=404, detail="Not registered as influencer")
...
@api_router.post("/influencers/instagram/webhook")
async def instagram_webhook(request: Request):
    """
    Webhook endpoint for Instagram comment notifications (MOCKED)
    In production, this would receive webhook events from Meta
    """
    # For demo purposes - simulate receiving a comment and triggering DM
    ...
    # Get influencer to check rate limits
...
@api_router.get("/influencers/wallet/transactions", response_model=List[WalletTransactionResponse])
async def get_wallet_transactions(user: Dict = Depends(get_current_user), skip: int = 0, limit: int = 50):
    """Get influencer wallet transactions"""
    influencer = await db.influencers.find_one({"user_id": user["user_id"]}, {"_id": 0})
    if not influencer:
        raise HTTPException(status_code=404, detail="Not registered as influencer")
...
@api_router.get("/influencers/wallet/balance")
async def get_wallet_balance(user: Dict = Depends(get_current_user)):
    ...
    return {
        ...
        "pending_withdrawals": 0,  # Calculate from withdrawals
        "min_withdrawal_amount": MIN_WITHDRAWAL_AMOUNT
    }

@api_router.post("/influencers/wallet/withdraw", response_model=WithdrawalResponse)
...
@api_router.get("/influencers/withdrawals", response_model=List[WithdrawalResponse])
...
@api_router.get("/admin/withdrawals", response_model=List[WithdrawalResponse])
...
@api_router.get("/influencers/referral-links")
async def get_influencer_referral_links(user: Dict = Depends(get_current_user)):
    """Get all referral links for influencer"""
    influencer = await db.influencers.find_one({"user_id": user["user_id"]}, {"_id": 0})
    if not influencer:
        raise HTTPException(status_code=404, detail="Not registered as influencer")
...
@api_router.get("/admin/analytics/influencers")
async def get_influencer_analytics(admin: Dict = Depends(get_admin_user)):
    ...
    for inf in influencers:
        # Get orders attributed to this influencer
        orders = await db.orders.find({
...
@api_router.get("/admin/analytics/payouts")
