...
export const ProductCard = ({ product }) => {
  ...
  return (
    ...
    >
      <div className={`relative aspect-[3/4] overflow-hidden bg-neutral-100 ${isOutOfStock ? "opacity-60" : ""}`}>
        <img
        ...
        <div className="absolute top-3 left-3 flex flex-col gap-1.5">
          {product.is_limited_edition && (
            <span className="font-mono text-[10px] uppercase tracking-widest bg-gold text-black px-2.5 py-0.5">Limited</span>
          )}
          {discount > 0 && (
            <span className="font-mono text-[10px] uppercase tracking-widest bg-black text-white px-2.5 py-0.5">-{discount}%</span>
          )}
          {product.stock > 0 && product.stock < 10 && (
            <span className="font-mono text-[10px] uppercase tracking-widest bg-red-500 text-white px-2.5 py-0.5">Low Stock</span>
          )}
          {isOutOfStock && (
            <span className="font-mono text-[10px] uppercase tracking-widest bg-neutral-700 text-white px-2.5 py-0.5">Out of Stock</span>
          )}
        </div>
        ...
        <button
          ...
          className="absolute top-3 right-3 w-9 h-9 bg-white/90 backdrop-blur-sm rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-300 hover:bg-gold hover:text-black"
        ...
        <div className="absolute bottom-0 left-0 right-0 p-3" onClick={e => e.preventDefault()}>
          <AnimatePresence mode="wait">
            {isOutOfStock ? (
              <motion.div
                ...
                className="bg-neutral-400 text-white text-center py-2.5 rounded-lg text-xs font-bold uppercase tracking-wider"
              >
                Out of Stock
              </motion.div>
            ) : cartQty > 0 ? (
              <motion.div
                ...
                className="flex items-center justify-between bg-black rounded-lg overflow-hidden shadow-lg"
                data-testid={`qty-controller-${product.product_id}`}
              >
                <button
                  onClick={handleDecrease}
                  className="w-11 h-10 flex items-center justify-center text-white hover:bg-neutral-700 transition-colors active:scale-90"
                ...
                <motion.span
                  ...
                  initial={{ scale: 1.3 }}
                  animate={{ scale: 1 }}
                  className="text-gold font-bold text-sm min-w-[32px] text-center"
              ...
              <motion.button
                ...
                disabled={adding}
                className="w-full bg-black/90 backdrop-blur-sm text-white py-2.5 rounded-lg text-xs font-bold uppercase tracking-wider flex items-center justify-center gap-2 hover:bg-gold hover:text-black transition-all opacity-0 group-hover:opacity-100 active:scale-95 disabled:opacity-50"
      ...
      <div className="pt-3 space-y-1.5">
        ...
        {product.average_rating > 0 && (
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-[10px] text-neutral-400">({product.review_count || 0})</span>
          </div>
        )}
