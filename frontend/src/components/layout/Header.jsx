    ...
    >
      <div className="max-w-7xl mx-auto px-4 md:px-8">
        <div className="flex items-center justify-between h-16 md:h-20">
          ...
          <Sheet open={isMobileMenuOpen} onOpenChange={setIsMobileMenuOpen}>
            ...
            <SheetContent side="left" className="w-80 bg-white">
              <nav className="flex flex-col gap-4 mt-8">
                ...
                {user ? (
                  <>
                    <Link to="/profile" onClick={() => setIsMobileMenuOpen(false)} className="text-lg py-2">My Account</Link>
                    <Link to="/orders" onClick={() => setIsMobileMenuOpen(false)} className="text-lg py-2">Orders</Link>
                    <Link to="/wishlist" onClick={() => setIsMobileMenuOpen(false)} className="text-lg py-2">Wishlist</Link>
                    <Link to="/influencer" onClick={() => setIsMobileMenuOpen(false)} className="text-lg py-2">Influencer Hub</Link>
                    {user.role === "admin" && (
                      <Link to="/admin" onClick={() => setIsMobileMenuOpen(false)} className="text-lg py-2 text-gold">Admin Dashboard</Link>
                    )}
                    <button onClick={() => { logout(); setIsMobileMenuOpen(false); navigate("/"); }}
                      className="text-lg py-2 text-left text-red-500">Logout</button>
                  </>
                ) : (
                  <Link to="/auth" onClick={() => setIsMobileMenuOpen(false)} className="text-lg py-2">Sign In</Link>
                )}
              </nav>
            </SheetContent>
          </Sheet>
          ...
          {/* Logo */}
          <Link to="/" className={`font-serif text-2xl md:text-3xl font-bold tracking-tight ${textColor}`} data-testid="logo">
            PIGMA
          </Link>
              ...
              {user ? (
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <Button variant="ghost" className={`${textColor} hover:text-gold flex items-center gap-2`}
                ...
                <Button variant="ghost" className={`${textColor} hover:text-gold`}
